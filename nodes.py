...
class FacesDetectorNode:
    ...
    def draw_masked_image(
        self, transformed_image: Image, faces: list[detector.Response]
    ) -> Image:
    ...
    def recognize(self, image: torch.Tensor) -> torch.Tensor:
        ...
        transformed_masked_image = self.draw_masked_image(transformed_image, faces)
        tensor = T.PILToTensor()(transformed_masked_image).permute(1, 2, 0)[None,]
        return (tensor,)
